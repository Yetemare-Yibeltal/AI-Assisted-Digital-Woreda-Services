...
const sheetVariants = cva(
  "fixed z-50 gap-4 p-6 glass-heavy border-border/20 shadow-2xl transition-all duration-300 ease-in-out",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, showClose = true, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {showClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 opacity-50 hover:opacity-100 transition-opacity focus:outline-none focus:ring-2 focus:ring-primary/50">
          <X className="h-5 w-5" />
